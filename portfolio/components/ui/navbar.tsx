...
export default function Navbar() {
    ...
    const tab2click = (href: string) => {
        gsap.to(indpill.current, {width: "99.6333px", left: "94px", duration: 0.4, ease: "power2.out"})
        gsap.to(indglow.current, {marginLeft: "134px", duration: 0.4, ease: "power1.out", onComplete: () => router.push(href)})
    }

    return (
        <nav className="pt-[24px] pl-[48px] pr-[48px] flex items-center justify-between sticky top-0">
            ...
            <div id="socials-wrapper" className="flex items-center gap-2">
                <Link href="" ref={linkedinref} className="flex text-[14px] tracking-wider font-normal p-3 pl-[16px] pr-[12px] gap-1 rounded-[24px] hover:bg-[#f2f2f20d] transition-colors duration-300 ease-[cubic-bezier(.165,.84,.44,1)]">
                ...
                <Link href="" ref={githubref} className="flex text-[14px] tracking-wider font-normal p-3 pl-[16px] pr-[12px] gap-1 rounded-[24px] hover:bg-[#f2f2f20d] transition-colors duration-300 ease-[cubic-bezier(.165,.84,.44,1)]">
